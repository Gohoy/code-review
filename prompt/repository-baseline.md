...
1. 首先调用一次 `repository_sync`。它会固定 Git 快照，并确定性写入全部项目函数、可唯一解析的调用关系和已有覆盖率证据；不要用 `graph_create_candidate` 机械重写这些事实。最小读取目标仓库的 README、AGENTS、用户入口、路由、核心代码与测试，且不得复用 sdbp-review 自身模型的需求或设计语义。
...
6. 无法从仓库证据判断的产品含义必须建立 `UNRESOLVED` Question，并用 `blocks` 关系阻断审批；禁止猜测，也禁止用“其他”“杂项”或 Repository 根节点兜底。存在真实函数时，至少一个关键 Symbol 必须由 Scenario 或其 `realized_by` 技术设计直接 `implemented_by`。
7. 完成前再次读取当前图，确认每个 Module 都有语义归属，并汇报结构归属函数数、语义归属函数数、直接函数归属数、未归属函数数、源码锚点迁移缺口、未解析调用和覆盖率未知；任何缺口都不是自动删除依据。
