...
11. 当前 MR 修改的范围内，只要存在无法扫描、无法锚定或无法追踪到批准场景的生产代码，就阻断自动 Review。
12. 未改动的历史缺口保留在图中并计入债务，但不阻断无关 MR。
...
页面可启动仓库基线任务：扫描器确定性写入全部项目函数与可唯一解析的调用关系，刷新时保留仍指向现存稳定函数 ID 的语义映射；覆盖率导入器读取 coverage.py JSON 或 LCOV 并生成 `OBSERVED` 验证证据。Agent 只补充 `implemented_by` 等语义映射；未映射函数、动态调用缺口和未知覆盖率始终保持可见。
