...
页面可启动仓库基线任务：扫描器确定性写入全部项目函数与可唯一解析的调用关系，刷新时保留稳定 ID 的语义映射；函数 ID 变化时，只按唯一的路径与限定名源码锚点迁移既有直接映射，歧义或无匹配锚点作为缺口汇报。覆盖率导入器读取 coverage.py JSON 或 LCOV 并生成 `OBSERVED` 验证证据。Agent 为每个 Module 补充 `implemented_by`，函数继承模块归属并可用直接关系细化；需求存在直接函数映射时，代码链优先展示这些函数及其技术调用后继，否则才展开模块兜底。未归属函数、动态调用缺口和未知覆盖率始终保持可见。
